...
class JdbcTest {
    ...
    @BeforeAll static void prepararSchema() throws Exception {
        String schema = "teste_mod29_" + UUID.randomUUID().toString().replace("-", "");
    ...
    @Test void clienteCompletaCrudComBuscaDeTodosEUpdate() throws Exception {
        Cliente c = clientes.cadastrar(new Cliente(0,"00000000001","Ana Exemplo"));
        assertTrue(c.id()>0); assertEquals(c,clientes.buscar(c.id()).orElseThrow());
        assertTrue(clientes.buscarTodos().contains(c));
        Cliente alterado = new Cliente(c.id(),c.cpf(),"Ana Atualizada"); clientes.atualizar(alterado);
    ...
    @Test void produtoCompletaCrud() throws Exception {
        Produto p = produtos.cadastrar(new Produto(0,"CRUD","Caderno",new BigDecimal("15.90")));
        assertEquals(p,produtos.buscar(p.id()).orElseThrow()); assertTrue(produtos.buscarTodos().contains(p));
        Produto alterado = new Produto(p.id(),p.codigo(),"Caderno grande",new BigDecimal("18.90")); produtos.atualizar(alterado);
    ...
    @Test void protegeCodigoDuplicado() throws Exception {
        Produto p = new Produto(0,"DUP","Teste",new BigDecimal("1.00"));
    ...
    @Test void parametroComApostrofoNaoViraSql() throws Exception {
        Produto p = produtos.cadastrar(new Produto(0,"ASP","D'Água; --",new BigDecimal("1.00")));
    ...
    @Test void atualizarRegistroAusenteInformaFalha() {
        assertThrows(NoSuchElementException.class,()->clientes.atualizar(new Cliente(-1,"00000000099","Ausente")));
        assertThrows(NoSuchElementException.class,()->produtos.atualizar(new Produto(-1,"AUS","Ausente",BigDecimal.ONE)));
    }
}
